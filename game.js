	...
	for (i = h; i-- > 0;) {
		...
		for (j = w; j-- > 0;) {
			...
			d.onclick = function() {
				...
				if (!o)
				do
				if (a[y = (Math.random() * h) | 0][x = (Math.random() * w) | 0] < 1 && (Math.abs(this.w - x) > 1 || Math.abs(this.h - y) > 1) && z--) a[y][x] = 1;
				while (z);
				if (!a[this.h][this.w]) N(this);
				else if (f = "Lose")
				for (i = h; i-- > 0;)
				for (j = w; j-- > 0;)
				if (a[i][j]) t.rows[i].cells[j][v] = "※"
			};
...
function N(c) {
	...
	if (~c[v].indexOf("#") && f++) T();
	if (!(c[v] = (a[y = c.h][(x = c.w) - 1] > 0) + (a[y][x + 1] > 0) + (y > 0 && a[y - 1][x - 1] > 0) + (y > 0 && a[y - 1][x] > 0) + (y > 0 && a[y - 1][x + 1] > 0) + (y < h - 1 && a[y + 1][x - 1] > 0) + (y < h - 1 && a[y + 1][x] > 0) + (y < h - 1 && a[y + 1][x + 1] > 0)) && (c[v] = "-"))
	for (c.i = 9; c.i-- > 0;)
